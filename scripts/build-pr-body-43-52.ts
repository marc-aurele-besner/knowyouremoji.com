#!/usr/bin/env bun
/**
 * Build the PR body for the third 250-combo PR (batches 43-52).
 * Reads scripts/batch-43-52-plan.json and emits markdown grouped by family.
 *
 * Usage: bun run scripts/build-pr-body-43-52.ts > pr-body.md
 */

import fs from 'fs';
import path from 'path';
import type { ComboPlanEntry } from './plan-batch-43-52';

const planPath = path.join(process.cwd(), 'scripts', 'batch-43-52-plan.json');
...
const FAMILY_TITLE: Record<string, string> = {
  'hearts-love': 'Hearts & Love III (25 combos)',
  'smileys-emotion': 'Smileys & Emotion Faces III (25 combos)',
  'hands-body': 'Hand Gestures & Body Language III (25 combos)',
  animals: 'Animals & Creatures III (25 combos)',
  'food-drink': 'Food & Drink III (25 combos)',
  'travel-places': 'Travel, Places & Adventure III (25 combos)',
  'sports-music': 'Sports, Music & Activities III (25 combos)',
  'weather-nature': 'Weather, Nature & Seasons III (25 combos)',
  'objects-tech': 'Objects, Tools & Tech III (25 combos)',
  'symbols-effects': 'Symbols, Sparkles & Effects III (25 combos)',
};
...
// Compute before/after category counts.
// "Before" is the existing 650 combos minus the 250 we're adding in this PR.
...
lines.push('- Total combos in `src/data/combos/`: 650 existing + 250 new = **900**');
lines.push(
