#!/usr/bin/env bun
/**
 * Per-batch deep-tier combo content writer for batches 43-52 (third 250-combo PR).
 *
 * Usage:
 *   bun run scripts/generate-deep-content-combos-43-52.ts <family-name>
 *   bun run scripts/generate-deep-content-combos-43-52.ts            # list families
 *
 * Reads scripts/batch-43-52-plan.json, writes 25 new combo JSON files into
...
import type { ComboPlanEntry, Family } from './plan-batch-43-52';
...
function main() {
  ...
  if (!family) {
    ...
    console.log('\nUsage: bun run scripts/generate-deep-content-combos-43-52.ts <family-name>');
    return;
  }
  writeBatch(family);
}
