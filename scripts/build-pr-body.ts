#!/usr/bin/env bun
/**
 * Build the PR body for the 250-combo PR.
 * Reads scripts/batch-23-32-plan.json and emits markdown grouped by family.
 *
 * Usage: bun run scripts/build-pr-body.ts > pr-body.md
 */
...
const FAMILY_TITLE: Record<string, string> = {
  'hearts-love': 'Hearts & Love (25 combos)',
  'smileys-emotion': 'Smileys & Emotion Faces (25 combos)',
  'hands-body': 'Hand Gestures & Body Language (25 combos)',
  animals: 'Animals & Creatures (25 combos)',
  'food-drink': 'Food & Drink (25 combos)',
  'travel-places': 'Travel, Places & Adventure (25 combos)',
  'sports-music': 'Sports, Music & Activities (25 combos)',
  'weather-nature': 'Weather, Nature & Seasons (25 combos)',
  'objects-tech': 'Objects, Tools & Tech (25 combos) — closes the work-category gap',
  'symbols-effects': 'Symbols, Sparkles & Effects (25 combos)',
};
...
// "Before" is the original 150 existing combos (recomputed by reading the existing
// JSON files that pre-date this PR).
function countBefore(): Record<string, number> {
  ...
  for (const f of fs.readdirSync(repoRoot)) {
    ...
    if (json.slug.includes('::')) continue; // index placeholder, if any
    // Heuristic: a slug from this PR is one we created in scripts/plan-batch-23-32.ts.
    let isOurs = false;
...
lines.push('- Total combos in `src/data/combos/`: 150 existing + 250 new = **400**');
lines.push(
